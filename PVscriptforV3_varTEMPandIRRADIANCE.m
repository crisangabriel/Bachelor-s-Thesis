...
for i = 1:length(irradiance_levels)
    ...
    %     P_valid(:,i) = P_results(:,i);
     ...
     %if ~isempty(V_valid) && ~isempty(I_valid) && ~isempty(P_valid)
        subplot(2,1,1);
        plot(V_valid, I_valid, 'DisplayName', ['Irradiance = ' num2str(irradiance_levels(i)) ' W/m^2']);
        hold on;
        ylabel('Current (A)');
        xlabel('Voltage (V)');
        ...
        subplot(2,1,2);
        plot(V_valid, P_valid, 'DisplayName', ['Irradiance = ' num2str(irradiance_levels(i)) ' W/m^2']);
        hold on;
        ylabel('Power (W)');
        xlabel('Voltage (V)');
